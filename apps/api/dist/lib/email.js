...
exports.sendOrderShipped = sendOrderShipped;
exports.sendOrderDelivered = sendOrderDelivered;
exports.sendOrderFailed = sendOrderFailed;
...
function getResend() {
    ...
    if (!env.RESEND_API_KEY)
        return null;
    if (!resendClient) {
        resendClient = new resend_1.Resend(env.RESEND_API_KEY);
    }
    return resendClient;
}
...
async function sendOrderDelivered(email, order) {
    const env = (0, env_1.getEnv)();
    const client = getResend();
    if (!client)
        return;
...
async function sendOrderFailed(email, order) {
    const env = (0, env_1.getEnv)();
    const client = getResend();
    if (!client)
        return;
