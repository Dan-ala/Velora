...
function baseUrl(): string {
  return getEnv().NODE_ENV === 'production' ? BASE_URL : TEST_URL;
}

async function authToken(): Promise<string | null> {
  const env = getEnv();
  if (!env.INTERRAPIDISIMO_USERNAME || !env.INTERRAPIDISIMO_PASSWORD) return null;
...
export class InterrapidisimoProvider implements IShippingProvider {
  ...
  async getTracking(guideNumber: string) {
    const token = await authToken();
  ...
  async cancelShipment(guideNumber: string): Promise<boolean> {
    const token = await authToken();
  ...
  async getRates(destination: string, weight: number) {
    return [
